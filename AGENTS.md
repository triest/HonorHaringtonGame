...
## 56.3 Full tactical command UI target (adopted from user-supplied ChatGPT spec, 2026-09-24) -- REVISES/EXTENDS 56.2, sets the real MVP acceptance bar

**Why:** after seeing the §56.2 result live, the user supplied a
detailed UI/UX specification (originally drafted with ChatGPT, saved
as this project's CHATGPT.md, now folded into this ТЗ verbatim below
as the authoritative section) plus a two-panel visual reference image
at `docs/reference/tactical_command_ui_reference.png` (fleet-view
squadron-list + tactical-plot-minimap layout on top, closer 3D
command-view with weapon/order panels on the bottom -- both READ THIS
IMAGE, it is the actual visual target, not just the text below). This
supersedes 56.2's more minimal "one ship, one plot panel" scope: the
real target is full hierarchical fleet command (Fleet -> Task Force ->
Squadron -> Division/Formation -> Ship), multi-object mouse selection,
a contextual order/weapons UI, and combined 3D + tactical-plot views.
56.2's already-implemented pieces (plot base, IFF-colored contacts,
missile tracks, mouse click-to-select, canon-scale distance
convention) are NOT thrown away -- they are the foundation this section
builds on. Where 56.2 and this section conflict on a *detail* (e.g.
"one selected ship" vs "multi-select"), this section (56.3) wins, being
the more recent and more specific decision.

**MANDATORY MVP acceptance test (§1.10.14 below) is now this
project's actual definition of "playable" for the command-UI part of
the vertical slice** -- do not report any part of this closed until a
user can do all 15 of those steps almost entirely with the mouse.

### 56.3 full text (verbatim from CHATGPT.md, adopted as ТЗ)

## 1.10. ОСНОВНОЙ GAMEPLAY LOOP — TACTICAL STRATEGY / COMMAND VIEW

Игра является **3D тактической стратегией управления космическим соединением**, а НЕ симулятором управления отдельным космическим кораблём.

Игрок находится в роли командующего и действует по схеме:

**НАБЛЮДАТЬ → ВЫБИРАТЬ → ОТДАВАТЬ ПРИКАЗЫ → КОНТРОЛИРОВАТЬ ИСПОЛНЕНИЕ → КОРРЕКТИРОВАТЬ ПРИКАЗЫ.**

Игрок НЕ должен постоянно "летать" одним кораблём клавиатурой.

### 1.10.1. 3D COMMAND VIEW

Основной режим — свободная командная камера над полем боя.

Камера должна позволять:

* смотреть на бой сверху под большим углом;
* полностью переходить к практически вертикальному виду "сверху";
* вращать камеру вокруг поля боя;
* приближать/отдалять поле боя колёсиком;
* быстро центрироваться на выбранном корабле, группе или контакте;
* видеть тактическую геометрию боя в трёх измерениях.

**Важно:** камера не привязана к кабине или корпусу корабля.

Игрок должен ощущать себя наблюдателем-командиром, управляющим боем с тактической карты.

### 1.10.2. МАСШТАБ

Поле боя должно поддерживать огромный пространственный масштаб Honorverse.

На одном экране потенциально должны существовать:
...
* ракетные группы;
* отдельные ракеты;
* миллионы километров дистанции между соединениями.

Нельзя использовать физический 1:1 визуальный масштаб для отображения кораблей.

Используется **символическое представление масштаба**:

* корабли имеют читаемый визуальный размер;
* расстояния между ними соответствуют симуляции;
* реальные дистанции всегда отображаются числовым значением;
* дальние объекты автоматически остаются видимыми на tactical plot;
* zoom изменяет представление, но не физику.

Таким образом игрок может одновременно видеть:

`Корабль A ← 1 850 000 km → Корабль B`

и при увеличении:

`Эскадра → отдельные корабли → отдельные ракеты`.

### 1.10.3. ZOOM LEVELS

Необходимо обеспечить несколько естественных уровней масштаба:
...
* все соединения;
* общие направления движения;
* расстояния между силами;
* крупные группы ракет.
...
* отдельные missile tracks;
* counter-missiles;
* направление атаки;
* estimated time-to-intercept;
* направление конечного захода.

Переход между уровнями должен происходить исключительно изменением масштаба камеры/тактического представления.

### 1.10.4. ВЫБОР ОБЪЕКТОВ МЫШЬЮ

Основной способ управления — **левая кнопка мыши**.

Игрок может кликнуть:
...
* по formation marker;
* по sensor contact;
* по собственной эскадре;
* по противнику.

После выбора объект получает визуальное выделение.
...
**CTRL + LMB**
— добавить объект к выбору.

**SHIFT + LMB**
— расширить/изменить существующую группу согласно контексту интерфейса.

**LMB drag**
— выделить прямоугольником несколько объектов.

Выбранные объекты должны быть объединены в единую командную группу.

### 1.10.5. ИЕРАРХИЧЕСКОЕ УПРАВЛЕНИЕ

Игрок может управлять на разных уровнях:

**Fleet → Task Force → Squadron → Division/Formation → Ship**

Если выбрана группа, приказ получает соответствующий командный уровень.

Если выбран один корабль, приказ применяется только к нему.

Если выбрана formation:

* корабли сохраняют formation;
* относительные позиции сохраняются;
* индивидуальная автоматика продолжает работать;
* локальные угрозы могут вызывать предусмотренную ТЗ subordinate initiative.

Это должно использовать существующую архитектуру `CommandEchelon`, `FormationState` и `IndividualOrder`, а не создавать отдельную параллельную систему управления.

### 1.10.6. ПРИКАЗ ДВИЖЕНИЯ МЫШЬЮ

После выбора корабля/группы игрок может:

**RMB по точке пространства**

→ создать приказ движения к указанной позиции.

**LMB drag от выбранной группы**

→ задать направление/курс.

Направление должно визуально отображаться как:

* линия курса;
* стрелка;
* конечная точка;
* прогнозируемый vector;
* при необходимости — предполагаемая точка разворота.

Приказ не должен означать мгновенное перемещение.

Корабль продолжает подчиняться физике Honorverse:
...
* wedge;
* текущая velocity;
* существующие ограничения maneuver model.

### 1.10.7. ПРИКАЗЫ ПО ЦЕЛИ

После выбора собственного корабля/группы и указания вражеского объекта мышью появляется контекстное меню приказов.
...
Приказы должны использовать существующие `ShipCombatDirective`, `FormationOrder` и `IndividualOrder`.

Не создавать отдельную UI-only модель приказов.

### 1.10.8. ВЫБОР ОРУЖИЯ

Игрок должен иметь возможность выбрать, **какой тип вооружения использовать**.

Минимальный интерфейс:

**MISSILES**

* выбрать missile type;
* выбрать salvo size;
* выбрать target;
* выбрать throttle/profile, если доступно;
* FIRE.
...
* laser;
* graser;
* другие доступные mounted weapons.

**POINT DEFENSE**

* AUTO;
* HOLD;
* назначение приоритетной угрозы.

Выбор оружия должен зависеть от реально установленного на выбранном классе корабля вооружения.

Нельзя показывать игроку оружие, которого у корабля нет.

### 1.10.9. КОНТЕКСТНОЕ УПРАВЛЕНИЕ

Не делать интерфейс из десятков постоянно видимых кнопок.

После выбора объекта интерфейс показывает только релевантные команды.

Например:

**Выбран SD**

`MOVE | FORMATION | ATTACK | FIRE CONTROL | MISSILES | PD | DEFEND | HOLD`

**Выбрана ракета**

`TARGET | INTERCEPT | TRACK | INFO`

**Выбрана вражеская эскадра**

`ATTACK | FOCUS FIRE | TRACK | INFO`

### 1.10.10. ТАКТИЧЕСКИЙ ПЛОТ + 3D

Тактический plot и полноценное 3D-представление **не являются взаимоисключающими**.

Игрок должен иметь возможность:

1. управлять боем непосредственно в 3D;
2. открыть/развернуть tactical plot;
3. использовать plot для выбора целей и групп;
4. возвращаться к 3D для понимания пространственной геометрии.

Tactical plot должен отображать:

* собственные силы;
* обнаруженные вражеские контакты;
...
* planned course;
* weapon range;
* relevant defensive coverage.

### 1.10.11. НЕ ПРЕВРАЩАТЬ ИГРУ В RTS

Это НЕ классическая RTS с:

* добычей ресурсов;
* строительством базы;
* производством юнитов;
* постоянным кликом по юнитам;
* мгновенным перемещением;
* HP bars.

Это **оперативно-тактическое управление космическим боем**.

Главный ресурс игрока — информация, время, положение соединений и правильность приказов.

### 1.10.12. ОДНОВРЕМЕННОЕ УПРАВЛЕНИЕ МНОЖЕСТВОМ КОРАБЛЕЙ

Игрок должен иметь возможность:

1. выбрать несколько кораблей;
2. объединить их в группу;
3. отдать один приказ;
4. получить визуальное подтверждение;
5. наблюдать исполнение;
6. выбрать отдельный корабль внутри группы;
7. отдать ему индивидуальный override.

Пример:

**Выбрана эскадра из 8 кораблей**

→ `FOCUS FIRE` по одному SD противника.

После этого:

→ выбрать один CA;

→ `COVER` соседний корабль;

→ остальные 7 кораблей продолжают выполнять исходный приказ.

Именно такое поведение должно поддерживать идею formation mutual defensive coverage и subordinate initiative.

### 1.10.13. ВАЖНЕЙШЕЕ ТРЕБОВАНИЕ К GAMEPLAY

При запуске игры игрок должен практически сразу понимать:

> **"Я командую боем."**

а не:

> **"Я управляю одним космическим кораблём."**

Если первый игровой экран заставляет игрока искать WASD/стрелки для управления кораблём, это считается неверным UX для данного проекта.

Первичный input loop:

**Mouse → Select → Order → Observe → Adjust**

Keyboard shortcuts являются только вторичным способом ускорения уже знакомых команд.

### 1.10.14. MVP ACCEPTANCE TEST

Vertical slice считается действительно работающим только если пользователь без чтения документации способен:

1. открыть 3D поле боя;
2. посмотреть на него сверху;
3. приблизить/отдалить масштаб;
4. выбрать один корабль мышью;
5. выбрать несколько кораблей рамкой;
6. выбрать вражескую цель;
7. отдать приказ движения мышью;
8. отдать приказ атаки;
9. выбрать тип вооружения;
10. запустить missile salvo;
11. увидеть missile tracks;
12. изменить приказ одной группе;
13. выделить один корабль внутри группы;
14. дать ему индивидуальный приказ;
15. продолжить наблюдать бой с командной камеры.

Если это невозможно сделать практически полностью мышью, vertical slice НЕ считается соответствующим концепции Tactical Strategy.
