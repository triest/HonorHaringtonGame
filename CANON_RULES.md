...
## Двадцатая сверка -- убраны имена конкретных кораблей, добавлена методика логического выстраивания характеристик (§8.4)

Пользователь уточнил: имена отдельных кораблей не важны, важны только
классы и их характеристики. Изменения:

* §8.3 в AGENTS.md урезана -- убран раздел "конкретные именованные
  корабли" (HMS Thunderer/HMS Revenge/PNS Kerebin/PNS Yavuz), оставлен
  только состав вооружения Sultan-class/Warlord-class. CLOUD.md §1.6
  синхронизирован.
* Новая §8.4 -- не источник канона, а МЕТОДИКА (расширение §4.1):
  как честно доводить недостающую характеристику класса/оружия до
  реализуемого числа, если источник её не даёт, вместо произвольной
  круглой цифры. Кратко: якорь -- ближайший уже подтверждённый класс
  ТОГО ЖЕ типа (§8.2) И той же эпохи/поколения техники; масса -- по
  аналогу той же эпохи, а не по середине диапазона типа; ускорение --
  обратно пропорционально массе ВНУТРИ одной эпохи (с явным
  предупреждением: Sultan/Warlord/Agamemnon из §8.1/§8.3 НЕ ложатся на
  одну кривую между собой -- поколения компенсаторов сами растут между
  эпохами, так что метод работает только внутри одного эпохального
  якоря); число мортир/труб -- пропорционально массе/длине корпуса по
  соотношению ближайшего якоря, округление до чётного симметричного по
  борту числа; ХАРАКТЕРИСТИКИ САМОГО ОРУЖИЯ (дальность/ускорение
  ракеты, мощность боеголовки) никогда не выводятся из корабля-
  носителя -- это свойство записи оружия (§8.1), наследуемое кораблём
  без изменений. Любое выведенное число обязано фиксировать якорь и
  использованный шаг метода, иначе оно неотличимо от выдуманного.

## Двадцать первая сверка -- §56.2 UI/control correction (масштаб дистанций, тактический плот)

Источник — уже задокументированные в §8.1 канон-числа (не новая
сверка, а применение уже подтверждённых цифр к архитектуре UI):
эффективная дальность энергооружия ~3,600,000 км, силовые огибающие
ракет 15,000,000-63,000,000 км (AGENTS.md §8.1, строки 317-318/
326-329). Вывод: первая версия вертикального среза (§56.1) отрисовывала
бой на дистанции буквально в десятки метров между моделями — прямое
противоречие этим уже подтверждённым канон-цифрам, обнаруженное
пользователем при живом тесте собранного .exe ("недозвёздные войны").
Статус: CANON-числа сами по себе не новые (уже в §8.1), но их
архитектурное следствие для UI — новое явное решение пользователя
(2026-09-23): дистанции и HUD должны символически отражать
канон-масштаб (плот с иконками/векторами, а не буквальный 1:1
рендер), управление — мышью, а не хоткеями. Технический метод
(символическая компрессия расстояния при сохранении отображаемых
цифр дальности) зафиксирован как INTERPRETATION в ASSUMPTIONS.md.

## Двадцать вторая сверка -- §56.3 полный tactical command UI (не канон-сверка, design-адопция)

Не источник для проверки канон-чисел: это фиксация того, что §56.3
(AGENTS.md) и §1.10 (CLOUD.md) приняты из пользовательской UI-
спецификации (CHATGPT.md + референс-картинка
docs/reference/tactical_command_ui_reference.png), а не из текста
книг. Канон-числа дистанций/вооружения, уже подтверждённые в §8.1 и
использованные в §56.2, остаются в силе и здесь -- §56.3 меняет UI/UX
поверх той же симуляции, не физику/цифры. См. ASSUMPTIONS.md для
полной формулировки INTERPRETATION-статуса.
