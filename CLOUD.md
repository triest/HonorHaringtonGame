...
* **Crossing the T (§41.1):** maneuver so the enemy's sector toward us is BOW/STERN (only bow/stern-arc weapons live, plus its 15° bow/stern wedge gap exposed) while our sector toward them stays PORT/STARBOARD (full broadside live). Uses existing `AttackGeometry.Sector`/`broadside_arc()`/`BOW_STERN_ACUTE_ANGLE_HALF_WIDTH_RAD` -- OPEN, no intercept-vector maneuvering AI exists yet.
